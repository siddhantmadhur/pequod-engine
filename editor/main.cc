...
void sokol_init() {
  ...
  if (home_dir) {
    auto &io = ImGui::GetIO();
    ui_settings_path =
        std::format("{}/pequod/engine_ui_settings.ini", home_dir);
    io.IniFilename = ui_settings_path.c_str();
  }
};

void sokol_frame_cb() { pequod_editor_main.sokol_frame_cb(); };
