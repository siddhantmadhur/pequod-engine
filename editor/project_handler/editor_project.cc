...
void ProjectSelectionScene::OnInitialLoad() {
  ...
  if (home_dir_env) {
    current_project_handler =
        std::make_unique<ProjectHandler>(ProjectHandler(home_dir_env));
...
void ProjectSelectionScene::QuitProgram() { sapp_quit(); }
...
void ProjectSelectionScene::OpenProject() {
  ...
  explorer.Initialize();
  object_tree =
      std::make_unique<ObjectTree>(current_project_handler->project_path,
                                   this->object_manager, this->selected_entity);
  object_tree->Initialize();

  object_properties = std::make_unique<ObjectPropertiesPanel>(
      this->selected_entity, this->object_manager);
...
void ProjectSelectionScene::OnFrameUpdate() {
  ...
  if (show_new_project_window) {
    ...
    // ImGui::SetNextWindowPos({sapp_widthf() / 2.0f, sapp_heightf() / 2.0f}, 0,
    // {0.5f, 0.5f});
    if (ImGui::Begin("Create New Project", &show_new_project_window,
                     ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse |
                         ImGuiWindowFlags_AlwaysAutoResize |
                         ImGuiWindowFlags_NoDocking)) {
      ImGui::InputText("Project Name", new_project_name, STR_LEN,
                       ImGuiInputTextFlags_CharsNoBlank, nullptr, nullptr);
      ImGui::InputText("Directory", project_directory, STR_LEN,
                       ImGuiInputTextFlags_CharsNoBlank, nullptr, nullptr);
  ...
  if (open_prev_project_window) {
    ...
    if (ImGui::Begin("Open Project", &open_prev_project_window,
                     ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse |
                         ImGuiWindowFlags_AlwaysAutoResize |
                         ImGuiWindowFlags_NoDocking)) {
      ...
      for (int i = 0; i < projects.size(); i++) {
        ...
        if (ImGui::Selectable(
                reinterpret_cast<const char *>(projects[i].c_str()),
                is_selected)) {
          selected_index = i;
        }
...
void ProjectSelectionScene::OnTickUpdate() {
  if (IsKeyPressed(SAPP_KEYCODE_LEFT_CONTROL) ||
      IsKeyPressed(SAPP_KEYCODE_RIGHT_CONTROL)) {
