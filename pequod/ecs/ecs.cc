...
std::unordered_map<std::type_index,
                   std::unordered_map<entity_id, std::shared_ptr<TProperty> > >
    ECS::properties;
...
void ECS::addMesh(entity_id id, std::shared_ptr<Mesh> mesh) {
  ...
  PDebug::log("addMesh: id={} vertices={} indices={} scale=({},{},{})", id,
              mesh->vertices.size(), mesh->indices.size(), mesh->GetScale().x,
              mesh->GetScale().y, mesh->GetScale().z);
  ...
  this->vertices.insert(vertices.end(), mesh->vertices.begin(),
                        mesh->vertices.end());
  this->indices.insert(indices.end(), mesh->indices.begin(),
                       mesh->indices.end());
}
...
void ECS::render(Camera &cam, float delta_t) {
  ...
  if (render_log_count < 3) {
    PDebug::log("render: max_id={} total_vertices={} total_indices={}", max_id,
                vertices.size(), indices.size());
    render_log_count++;
  }
  for (int i = 0; i < max_id; i++) {
    auto &meshes = GetProperties<Mesh>();
    // [CLAUDE] TODO: meshes[i] silently creates a default entry if key doesn't
    // exist — use .find() or .contains() check
    auto mesh = meshes[i];
    auto &positions = GetProperties<Transform>();
    ...
    // std::cout << mesh->indices_id << ", " << mesh->indices.size() <<
    // std::endl;
...
/**
#define ADD_PROPERTY(prop) template void ECS::AddProperty<prop>(entity_id,
std::shared_ptr<prop>) ADD_PROPERTY(Position); ADD_PROPERTY(Mesh);
**/
}  // namespace Pequod
