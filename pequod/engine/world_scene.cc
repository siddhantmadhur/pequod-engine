...
void WorldScene::Initialize() {
  ...
  **/
  ...
  index_buffer.usage = {.index_buffer = true, .dynamic_update = true};
  ...
  pass_action.colors[0] = {
      ...
      .clear_value = {bgColor.x, bgColor.y, bgColor.z, bgColor.a}};

  auto img_desc = sg_image_desc{.width = wall_texture.x,
...
void WorldScene::BeginRenderPass(float width, float height) {
  ...
  simgui_new_frame(simgui_frame_desc_t{
      .width = sapp_width(), .height = sapp_height(), .delta_time = delta_t});
...
void WorldScene::SetPlayerCamera(Camera &cam) { this->playerCamera = cam; }

Camera &WorldScene::GetPlayerCamera() { return this->playerCamera; }
...
sg_pass_action WorldScene::GetPassAction() { return this->pass_action; }
}  // namespace Pequod
